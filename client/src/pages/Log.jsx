...
const handleLogin = async (e) => {
  ...
  if (isVolunteer) {
    ...
    // Check if volunteer exists in Firestore first
    const volunteerDoc = await getDoc(doc(db, "volunteers", email));
    if (!volunteerDoc.exists()) {
      setErrorMsg("Volunteer not registered. Please contact admin.");
      return;
    }
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (email.endsWith("@jecrc.ac.in")) {
        setErrorMsg("Volunteers must login using email and shared password.");
        return;
      }
